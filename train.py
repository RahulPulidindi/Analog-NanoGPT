...
def trainv2(
    ...
    # use smaller batch size for analog training to avoid OOM
    if model_name.startswith("Analog"):
        analog_bs = 1
        train_loader = DataLoader(train_ds, batch_size=analog_bs, shuffle=True)
    else:
        train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True)
